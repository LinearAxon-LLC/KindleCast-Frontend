...
function PlanCard({ plan, onUpgrade, isLoading }: PlanCardProps) {
  ...
  return (
    <div
      className={`relative bg-white border-2 rounded-[16px] p-6 transition-all duration-200 hover:shadow-lg ${
        isPopular
    ...
    >
      ...
      {isPopular && (
        <div className="absolute -top-3 left-1/2 transform -translate-x-1/2">
          <div className="bg-brand-primary text-white px-4 py-1 rounded-full text-[12px] font-semibold shadow-lg">
            Most Popular
          </div>
        </div>
      )}
      ...
      <button
        ...
        className={`w-full px-6 py-3 rounded-[8px] font-medium transition-all duration-150 active:scale-[0.98] ${
          isPopular
            ? "bg-brand-primary text-white shadow-lg hover:shadow-xl"
