...
export function useUserUsage(): UseUserUsageReturn {
  ...
  const getUsagePercentage = useCallback(
    (type: "basic" | "ai"): number => {
      if (!usage) return 0;

      const limit =
        type === "basic" ? usage.basic_monthly_limit : usage.ai_monthly_limit;
      const used =
        type === "basic" ? usage.used_basic_monthly : usage.used_ai_monthly;
...
export function useUsageDisplay() {
  const { usage, getUsagePercentage, isUnlimited, getRemainingUsage } =
    useUserUsage();

  const formatUsageText = useCallback(
    (type: "basic" | "ai"): string => {
  ...
  const getUsageStatus = useCallback(
    (type: "basic" | "ai"): "low" | "medium" | "high" | "unlimited" => {
