...
interface UseLinkProcessorReturn extends UseLinkProcessorState {
  submitLink: (url: string, format: string, customPrompt?: string) => Promise<void>;
  resetState: () => void;
}
...
export function useLinkProcessor(): UseLinkProcessorReturn {
  ...
  const submitLink = useCallback(async (url: string, format: string, customPrompt?: string) => {
    ...
    const validationError = validateLinkRequest(url, backendFormat, customPrompt);
    if (validationError) {
      setState({
        isLoading: false,
        isSuccess: false,
